$ErrorLogPreference = "C:\retry.txt"
...
function Set-InventoryInDatabase {
    ...
    PROCESS {
        foreach ($obj in $InputObject) {
            $query = "UPDATE computers_computer SET
        ...
        foreach ($computer in $ComputerName) {
            ...
            if ($passtest) {
                foreach ($drive in $data) {
                    ...
                    $object = New-Object -TypeName psobject -Property $properties
                    $object.psobject.TypeNames.Insert(1, 'LCO.SystemVolumeData')
                    Write-Output $object
...
Export-ModuleMember -Function Get-SystemData, Get-SystemVolumeData, Get-SystemServiceData, Get-ComputerNamesFromDatabase, Set-InventoryInDatabase, Set-ServicePassword, Set-ComputerState, Get-RemoteSmbShare
